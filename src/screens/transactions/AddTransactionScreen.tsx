...
const AddTransactionScreen: React.FC<any> = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (isEditing && existingTxn) {
      ...
      const isPredefined = CATEGORIES.some(c => c.name === existingTxn.category);
      if (isPredefined) {
        setCategory(existingTxn.category);
        setCustomCategoryName('');
      } else {
        setCategory('Custom');
        setCustomCategoryName(existingTxn.category);
      }
    }
  ...
  const validate = (): boolean => {
    ...
    if (!category) {
      ...
      if (!customCategoryName.trim()) {
        newErrors.customCategory = 'Enter custom category name';
      } else if (customCategoryName.trim().length < 2) {
        newErrors.customCategory = 'Must be at least 2 characters';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    const finalCategory = category === 'Custom' ? customCategoryName.trim() : category;
          ...
          <ScrollView
            ...
            {category === 'Custom' && (
              <InputField
                ...
                placeholder="e.g. Gym, Gifts, Pets"
